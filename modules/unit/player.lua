...
DFRL:NewMod("Player", 1, function()
    ...
    function Setup:CutOut()
        ...
        local function CreateCutoutEffect(statusBar, barType, unit)
            ...
            -- debugprint("Cutout frame created and added to animationFrames")
            return cutoutFrame
        end
        ...
        local function UpdateCutoutEffect(frame, unit)
            ...
            -- debugprint("UpdateCutoutEffect called - Unit: " .. unit .. ", ID: " .. unitID .. ", BarType: " .. frame.barType .. ", Current: " .. currentValue .. ", Max: " .. maxValue)

            if UnitIsDead(unit) or UnitIsGhost(unit) then
                -- debugprint("Unit is dead or ghost - setting lastValue and returning")
            ...
            if frame.lastUnitID ~= unitID then
                -- debugprint("ACTUAL UNIT CHANGED - Old ID: " .. (frame.lastUnitID or "nil") .. ", New ID: " .. unitID .. ", Setting lastValue to: " .. currentValue)
                frame.lastUnitID = unitID
                frame.lastValue = currentValue
            ...
            if not frame.initialized then
                -- debugprint("Frame not initialized - setting lastValue to: " .. currentValue)
                frame.lastValue = currentValue
                frame.lastUnitID = unitID
                frame.initialized = true
                return
            ...
            -- debugprint("Comparing values - LastValue: " .. (frame.lastValue or "nil") .. ", CurrentValue: " .. currentValue)
            ...
            if frame.lastValue and currentValue < frame.lastValue and maxValue > 0 then
                ...
                -- debugprint("TRIGGERING CUTOUT EFFECT - Lost: " .. (frame.lastValue - currentValue) .. ", LostPercent: " .. lostPercent .. ", CutoutWidth: " .. cutoutWidth)
            ...
            frame.lastValue = currentValue
            -- debugprint("Updated lastValue to: " .. currentValue)
        end
        ...
        local function HookUnitFrames()
            ...
            if targetHealth then
                ...
                targetHealth:SetScript("OnValueChanged", function()
                    debugprint("TARGET HEALTH OnValueChanged triggered")
            ...
            if targetMana then
                ...
                targetMana:SetScript("OnValueChanged", function()
                    debugprint("TARGET MANA OnValueChanged triggered")
